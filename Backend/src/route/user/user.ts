...
router.post('/', (req, res) => {
  ...
  if (!DataUserManager.create({ userId, userName, passwd, userThumbnail, email })) {
...
router.put('/', tokenRouter, (req, res) => {
  ...
  if (!DataUserManager.update({ userId, passwd, userName, userThumbnail, email })) {
    return res.json({ code: ResponseCode.invaildParameterType })
  }

  log.info(`Update user account (userId: "${userId}")`)
  log.info(`Update user account (userName: "${userName}")`)
